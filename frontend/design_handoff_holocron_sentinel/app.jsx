...
const { entities: SEED_ENTITIES, events: SEED_EVENTS, CRITICAL_LIMIT, generateLiveEvent } = window.HolocronData;
...
function App() {
  ...
  React.useEffect(() => {
    const onHash = () => setRoute(parseRoute(window.location.hash.replace(/^#\/?/, "") || "dashboard"));
  ...
  const [liveEvents, setLiveEvents] = React.useState(SEED_EVENTS.slice(0, 18).map((e) => ({ ...e })));
  ...
  function handleToggleStatus(id) {
    setEntities((cur) =>
      cur.map((e) => {
        ...
        return { ...e, status: newStatus, critical_events_count: newCount, updated_at: new Date().toISOString() };
      })
    );
  }
