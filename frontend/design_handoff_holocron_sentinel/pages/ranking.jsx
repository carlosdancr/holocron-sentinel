...
const RankingPage = ({ entities, onOpenEntity, criticalLimit }) => {
  ...
  const max = ranked[0]
    ? window === '7d'
      ? ranked[0].crit_7d
      : ranked[0].critical_events_count
    : 1
  const totalCrit = ranked.reduce(
    (s, e) => s + (window === '7d' ? e.crit_7d : e.critical_events_count),
    0,
  )
        ...
        <div className="page-header-row">
          <div>
            ...
            <div className="page-subtitle">
              Entidades com maior incidência de eventos críticos. Atualizado por agregação
              periódica.
            </div>
          </div>
          <div className="chip-group">
            <button
              className={'chip' + (window === '24h' ? ' on' : '')}
              onClick={() => setWindow('24h')}
            >
              24h
            </button>
            <button
              className={'chip' + (window === '7d' ? ' on' : '')}
              onClick={() => setWindow('7d')}
            >
              7 dias
            </button>
            <button
              className={'chip' + (window === '30d' ? ' on' : '')}
        ...
        <div className="kpis" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
          <div className="kpi hero">
            ...
            <div className="kpi-foot">
              {ranked[0]
                ? `${window === '7d' ? ranked[0].crit_7d : ranked[0].critical_events_count} eventos críticos`
                : 'sem dados'}
            </div>
          </div>
          <div className="kpi">
          ...
          <div className="card-header">
            <div>
              <div className="card-title">
                Top 12 — janela{' '}
                {window === '7d' ? '7 dias' : window === '24h' ? '24 horas' : '30 dias'}
              </div>
              <div className="card-subtitle">
                ...
                >
                  GET /entities/ranking?window={window}
                </code>
              </div>
            </div>
              ...
              {ranked.map((e, i) => {
                ...
                return (
                  <div
                    key={e.id}
                    className={
                      'rank-row ' + (i === 0 ? 'top-1' : i === 1 ? 'top-2' : i === 2 ? 'top-3' : '')
                    }
          ...
          <div className="scrim">
            ...
            {window === '7d'
              ...
              : window === '24h'
                ? 'últimas 24h'
                : 'últimos 30 dias'}{' '}
            · ordem decrescente por eventos críticos · agregação cacheada (TTL 30s)
          </div>
        </div>
      </div>
    </div>
