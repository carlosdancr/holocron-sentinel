...
function HourlyChart({ hourly }) {
  ...
  const hoverLabel = hover ? (hoursAgo === 0 ? 'última hora' : `há ${hoursAgo}h`) : ''
          ...
          <g className="chart-grid">
            {[0.25, 0.5, 0.75].map((p) => (
              <line
                ...
                x1={padX}
                x2={VBW - padX}
                y1={padTop + p * (VBH - padTop - padBot)}
                y2={padTop + p * (VBH - padTop - padBot)}
              />
            ))}
          </g>
...
const EntityDetailPage = ({ entity, events, criticalLimit, onBack, onToggleStatus, onGoForm }) => {
  ...
  const last24h = entityEvents.filter(
    (ev) => Date.now() - new Date(ev.created_at).getTime() < 24 * 3600 * 1000,
  )
  ...
  const hourly = React.useMemo(() => {
    const buckets = Array(24)
      .fill(0)
      .map(() => ({ info: 0, warning: 0, critical: 0, total: 0 }))
    const now = Date.now()
      ...
      <div className="page-header">
        ...
        <div className="page-header-row">
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <button className="btn btn-ghost btn-icon" onClick={onBack} title="Voltar">
          ...
          <div className="alert alert-error" style={{ marginBottom: 18 }}>
            ...
            <div>
              ...
              <div className="alert-desc">
                Limite de {criticalLimit} eventos críticos foi atingido em{' '}
                {formatDateTime(entity.updated_at)}. Novos eventos serão rejeitados até reativação
                manual.
              </div>
            </div>
          </div>
        ...
        {isNearLimit && (
          <div className="alert alert-warning" style={{ marginBottom: 18 }}>
            ...
            <div>
              ...
              <div className="alert-desc">
                {entity.critical_events_count} de {criticalLimit} eventos críticos. A próxima
            ...
            <div className="detail-summary">
              <div className="kpi">
                ...
                <div className="kpi-foot">
                  desde {formatDateTime(entity.created_at).split(',')[0]}
                </div>
              </div>
              <div className="kpi">
              ...
              <div className="kpi">
                ...
                <div className="kpi-foot" style={{ display: 'flex', gap: 12 }}>
                  <span>
                    <b style={{ color: 'var(--critical)' }}>{last24hCrit}</b> crit
                  </span>
                  <span>
                    <b style={{ color: 'var(--warning)' }}>{last24hWarn}</b> warn
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="card-header">
                <div>
                  ...
                  <div className="card-subtitle">
                    {entityEvents.length} eventos registrados para esta entidade.
                  </div>
                </div>
                    ...
                    <div key={ev.id}>
                      ...
                      >
                        ...
                        <button className="btn btn-ghost btn-sm btn-icon">
                          <Icon
                            name={expanded === ev.id ? 'chevron-down' : 'chevron-right'}
                            size={12}
                          />
                        </button>
                      </div>
              ...
              <div className="card-body" style={{ padding: '8px 18px' }}>
                <ul className="info-list">
                  <li>
                    ...
                    <span className="info-val">{entity.id}</span>
                  </li>
                  <li>
                  ...
                  <li>
                    <span className="info-key">Eventos críticos</span>
                    <span className="info-val">
                      {entity.critical_events_count} / {criticalLimit}
                    </span>
                  </li>
                  <li>
                    <span className="info-key">Criada em</span>
                    <span className="info-val">{formatDateTime(entity.created_at)}</span>
                  </li>
                  <li>
                    <span className="info-key">Atualizada em</span>
                    <span className="info-val">{formatDateTime(entity.updated_at)}</span>
                  </li>
                  <li>
                    <span className="info-key">Último evento</span>
                    <span className="info-val">{formatRelative(entity.last_event_at)}</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="card-body">
                <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.55 }}>
                  ...
                  <b style={{ color: 'var(--text)' }}>{criticalLimit} eventos críticos</b>. Eventos
