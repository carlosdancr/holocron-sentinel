...
const DashboardPage = ({
  ...
  const filtered = React.useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter((e) => e.name.toLowerCase().includes(q) || e.id.toLowerCase().includes(q))
    }
    if (statusFilter !== 'all') r = r.filter((e) => e.status === statusFilter)

    const sorted = [...r]
    if (sortBy === 'threat')
      sorted.sort((a, b) => b.critical_events_count - a.critical_events_count)
    if (sortBy === 'events') sorted.sort((a, b) => b.total_events - a.total_events)
    if (sortBy === 'recent')
      sorted.sort((a, b) => new Date(b.last_event_at) - new Date(a.last_event_at))
    ...
    <div className="page">
      ...
      <div className="page-body" style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        <div className="kpis">
          <div className="kpi hero">
            ...
            <div className="kpi-value">
              {kpis.active}
              <span style={{ fontSize: 18, opacity: 0.5 }}> / {kpis.total}</span>
            </div>
            ...
            <svg className="kpi-spark" width="80" height="32" viewBox="0 0 80 32" fill="none">
              <path
                d="M2 24 L12 22 L22 25 L32 18 L42 20 L52 12 L62 15 L72 8"
                stroke="currentColor"
                strokeWidth="1.5"
                fill="none"
                opacity="0.7"
          ...
          <div className="kpi">
            ...
            <div className="kpi-foot">≥ 70% do threshold ({criticalLimit})</div>
          </div>
          <div className="kpi">
        ...
        <div className="card">
                ...
                <tbody>
                  {filtered.map((e) => (
                    <tr key={e.id} className="clickable" onClick={() => onOpenEntity(e.id)}>
                      ...
                      <td>
                        <ThreatMeter count={e.critical_events_count} limit={criticalLimit} />
                      </td>
                      <td className="num" style={{ textAlign: 'right' }}>
                        {e.total_events.toLocaleString('pt-BR')}
                      </td>
                      <td className="muted" style={{ fontSize: 12.5 }}>
                        {formatRelative(e.last_event_at)}
                      </td>
          ...
          <div className="scrim">
            {filtered.length} de {entities.length} entidades · limite crítico: {criticalLimit}{' '}
            eventos
          </div>
        </div>
      </div>
