...
export function ChipGroup({ options, value, onChange }: ChipGroupProps) {
  return (
    <div className="flex gap-1 rounded-sm border border-border bg-surface-2 p-0.5">
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
          ...
          className={cn(
            'inline-flex h-7 cursor-pointer items-center gap-1.5 rounded-[4px] px-2.5 text-[12.5px] font-medium text-text-muted transition-[background,color,box-shadow] duration-120',
